...
class L8Image:
    ...
    def load_bands(self, bands=None, masked=True,
        ...
        assert len(bands) == 1 or (8 not in bands), \
            "Cannot load panchromatic band (B8) together with other bands because it has different resolution"
        ...
        if masked:
            ...
            mask = predbytiles.mask_2D_to_3D(mask, img.shape[-1])
